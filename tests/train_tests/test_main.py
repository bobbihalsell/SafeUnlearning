...
class TestTrainApp(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_config.__getitem__.side_effect = lambda key: {
            ...
            "train_cfg": {"lr": 0.001, "epochs": 2, "weight_decay": 0.0001},
            "wandb": {"run_id": "test-run", "project_name": "test-project"},
        }[key]

        # Patch OmegaConf.to_container to return our mock config
        self.omegaconf_patcher = patch(
            "omegaconf.OmegaConf.to_container", return_value=self.mock_config
        )
        ...
        self.setup_device_patcher = patch(
            "src.train.main.setup_device", return_value="cpu"
        )
