...
from eval_utils import (accuracy, normed_distance, l2_distance, normed_l2_distance, 
                        kld, visualize_evaluation_results)
    ...
    def evaluate_model_pairs(self, 
        ...
        for model_key, unl_model in self.models.items():
            if model_key != 'original':
                ...
                unl_results["normalized_distance"] = normed_distance(orig_model, unl_model)
                if self.verbose:
                    print(f"Normalized distance: {unl_results["normalized_distance"]:.4f}")
                # Normalized L2 distance
                unl_results["normalized_l2_distance"] = normed_l2_distance(orig_model, unl_model, self.device)
                if self.verbose:
                    print(f"Normalized L2 distance: {unl_results["normalized_l2_distance"]:.4f}")
                # KL divergence on each data split
                unl_results["kl_divergence"] = kld(orig_model, unl_model, dataloaders, self.device)
                print(f"kl_divergence between {model_key} model and the original:", end=" ")
