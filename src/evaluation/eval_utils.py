...
def accuracy(model, dataloaders, device, model_name="", verbose=False):
    ...
    for split_name, dataloader in dataloaders.items():
        ...
        if verbose:
            ...
            print(
                ", ".join(
                    [f"{key}: {value:.2f}%" for key, value in model_results.items()]
                )
            )
...
def plot_model_accuracies(results, output_dir="plots"):
    ...
    fig, axes = plt.subplots(
        1, len(datasets), figsize=(5 * len(datasets), 6), squeeze=False
    )
...
def plot_weight_distance_metrics(results, output_dir="plots"):
    ...
    fig, axes = plt.subplots(
        1, len(weight_metrics), figsize=(5 * len(weight_metrics), 6), squeeze=False
    )
...
def plot_kl_divergence(results, output_dir="plots"):
    ...
    for i, dataset in enumerate(datasets):
        ...
        for model in model_names:
            if (
                "kl_divergence" in unl_distances[model]
                and dataset in unl_distances[model]["kl_divergence"]
            ):
                valid_models.append(model)
