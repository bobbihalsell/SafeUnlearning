...
class DatasetInitializer(DatasetValidator):
    ...
    def _reinitialize_splits_dir(self):
        ...
        if os.path.exists(self.save_path):
            print(f"Clearing existing dataset split directory: {self.save_path}")
    ...
    def create_forget_retain_symlinks_by_random_n(
        self, train_dir: str, output_dir: str, forget_size: int, retain_size: int = None
    ):
        """
        ...
        forget_indices = np.random.choice(
            range(len(train_dataset)), size=forget_size, replace=False
        )
        ...
        if retain_size is not None:
            retain_indices = list(
                np.random.choice(
                    retain_indices, min(retain_size, len(retain_indices)), replace=False
                )
            )
    ...
    def create_forget_retain_symlinks_by_classes(
        self, train_dir: str, output_dir: str, forget_classes: list
    ):
        """
    ...
    def create_forget_retain_symlinks_by_class_number(
        self, train_dir: str, output_dir: str, forget_classes: dict
    ):
        """
        ...
        forget_classes = {
            str(label): str(count) for label, count in forget_classes.items()
        }
            ...
            if class_label in forget_classes:
                ...
                for file in os.listdir(class_dir):
                    try:
                        ...
                        print(f"Encountered invalid image file {file}. Skipping...")
                        continue
                ...
                if num_samples_to_forget > 0:
                    ...
                    forget_files = list(
                        np.random.choice(
                            all_files, size=num_samples_to_forget, replace=False
                        )
                    )
    ...
    def create_forget_retain_symlinks_by_filename(
        ...
        for label in os.listdir(train_dir):
            ...
            for fp in os.listdir(label_dir):
                ...
                if self.retain_filenames is None:
                    ...
                    src_file = os.path.abspath(os.path.join(train_dir, label, fp))
        ...
        if self.retain_filenames is None:
            ...
            forget_retain_filenames = self.forget_filenames + self.retain_filenames

        missing = set(forget_retain_filenames) - set(found)
        if missing:
            print(
                "Warning: Failed to find the following filenames you have "
    ...
    def del_parent_dir(self):
        if self.init_path == "tmp_dir":
            ...
            print(
                "Deleted the temporary directory holding the "
                "original training and test data."
            )


@hydra.main(version_base=None, config_path="../../configs", config_name="datasets")
