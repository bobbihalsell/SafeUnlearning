...
class KUnlearn(FinetuneUnlearner):
    """
    Implementation of layer-selective unlearning that freezes the first 
    k layers.
    
    This approach is based on the concept that different layers in neural 
    networks capture different levels of abstraction. By freezing the first k 
    layers and either:
    1. Fine-tuning only the remaining layers (cfk - Catastrophic Forgetting)
    2. Reinitializing and then fine-tuning the remaining layers 
        (euk - Exact Unlearning)
    ...
    def __init__(self, 
        ...
        Args:
            k (int): Number of layers to freeze from the beginning of the
                    model. These layers will maintain their original weights.
            ...
            method (str): Unlearning approach to use. Options:
                         - 'cfk': Catastrophic Forgetting - freeze first k 
                            layers and fine-tune the rest.
                         - 'euk': Exact Unlearning - freeze first k layers, 
                            reinitialize the rest, and then fine-tune.
        """
    ...
    def _freeze_first_k_layers(self, model):
        """
        Freezes the first k layers of the model to prevent updates during 
        training.
    ...
    def _reinitialize_weights(self, model):
        ...
        Args:
            ...
            method (str): The initialization method. Options:
                ...
                - "randn": Initializes weights with a standard normal 
                    distribution
    ...
    def unlearn(self,
        ...
        1. CFk (Catastrophic Forgetting): Freeze first k layers and fine-tune 
            the rest
        2. EUk (Exact Unlearning): Freeze first k layers, reinitialize 
            remaining layers, and then fine-tune

        Args:
            model (nn.Module): The original model to unlearn from
            data_dict (Dict[str, DataLoader]): Dictionary containing 
                dataloaders for different datasets. Must include 'retain' data
            reinit_method (str): Method to use for reinitializing weights in 
                EUk approach Options: 'zero', 'randn'
