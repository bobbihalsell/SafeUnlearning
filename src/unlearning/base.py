...
class BaseUnlearner:
    ...
    def initialize_optimizer(self, model: nn.Module, optimizer_name: str):
        ...
        if optimizer_name == "adam":
            ...
            raise ValueError(
                f"Only adam and sgd optimizers supported. Received {optimizer_name}."
            )
    ...
    def _evaluate_all_splits(
        ...
        for data_type, loader in data_dict.items():
            ...
            if self.verbose:
                print(
                    f"{data_type.capitalize()} Loss: {loader_loss:.4f} "
                    f"Acc: {loader_acc:.2f}%. "
                    f"Time: {elapsed:.1f} s",
                    end=" || ",
                )
    ...
    def _setup_unlearning(self, model, data_dict, **kwargs):
        ...
        self.logs.update({f"{data_type}_acc": [] for data_type in data_dict.keys()})
        self.logs.update({f"{data_type}_time": [] for data_type in data_dict.keys()})
        self._eval_initial_model(model, data_dict)

        self.optimizer = self.initialize_optimizer(
            unlearned_model, optimizer_name=self.optimizer
        )

        if self.epochs_per_lr_decay is not None and self.lr_decay_factor is not None:
    ...
    def _eval_initial_model(self, model, data_dict):
        ...
        for data_type in data_dict.keys():
            ...
            if self.verbose:
                print(
                    f"Initial {data_type.capitalize()} Loss: "
                    f"{split_loss:.4f}. "
                    f"Acc: {split_acc:.2f}%.",
                    end=" || ",
                )
    ...
    def _print_forward_pass_metrics(self, epoch_num, time_taken):
        ...
        print(
            f"Epoch {epoch_num + 1} Forward Pass Complete. "
            f"LR: {current_lr:.5f}. "
            f"Time taken: {time_taken:.1f} s"
        )

    def _log_forward_pass_time_in_wandb(self, epoch, time):
        wandb.log({"Epoch": epoch, "forward_pass_time": time})

    def _log_metrics_in_wandb(
        self,
        epoch: int,
        retain_loss: float,
        retain_acc: float,
