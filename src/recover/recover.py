...
class CachedGradientRecoveryPipeline:
    ...
    def get_gradients(self, model, data_loader, loss_fn=None, max_batches=None):
        """
        Calculate average gradients for the model on given data.
        Returns gradients averaged across BATCHES (not individual samples).
        Cached based on model and data characteristics.
        """
        cache_key = self._get_cache_key(
            "gradients",
                ...
                for name, param in model.named_parameters():
                    if param.grad is not None:
                        ...
                        if name not in accumulated_gradients:
                            # Clone and detach to avoid computation graph issues
                            accumulated_gradients[name] = param.grad.clone().detach()
                        else:
                            accumulated_gradients[name] += param.grad.clone().detach()
                    else:
                        # Initialize with zeros if gradient is None
                        if name not in accumulated_gradients:
                            accumulated_gradients[name] = torch.zeros_like(param)
                
                if not batch_has_gradients:
                    print(f"WARNING: No gradients computed for batch {batch_idx}")
        ...
        # Average gradients across batches (NOT samples)
        final_gradients = {}
        for name in accumulated_gradients:
            final_gradients[name] = accumulated_gradients[name] / num_batches_processed
        
        # Debug info and validation
        if self.verbose and final_gradients:
            # Check a few example gradients
            gradient_stats = []
            for name, grad in list(final_gradients.items())[:5]:  # Check first 5
                if grad is not None:
                    grad_norm = torch.norm(grad).item()
                    grad_mean = grad.mean().item()
                    gradient_stats.append((name, grad_norm, grad_mean))
            
            print(f"   ✅ Processed {num_batches_processed} batches ({total_samples_processed} total samples)")
            print(f"   ✅ Computed gradients for {len(final_gradients)} parameters")
            print(f"   📊 Example gradient statistics:")
            for name, norm, mean in gradient_stats:
                print(f"      {name[:30]:30s}: norm={norm:.6f}, mean={mean:.6f}")
        
        # Sanity check: make sure gradients aren't all tiny
        ...
        for name, grad in final_gradients.items():
            if grad is not None:
                ...
                if grad_norm > 1e-8:  # Not essentially zero
                    non_zero_gradients += 1
        
        if non_zero_gradients == 0:
            print(f"⚠️  WARNING: All gradients are essentially zero! Max norm: {max_gradient_norm:.2e}")
            print(f"    This suggests the models might be identical or there's a computation issue.")
        elif max_gradient_norm < 1e-6:
            print(f"⚠️  WARNING: Gradients are very small! Max norm: {max_gradient_norm:.2e}")
            print(f"    This might indicate very similar models or numerical precision issues.")
        else:
            if self.verbose:
                print(f"   ✅ Gradients look reasonable: {non_zero_gradients}/{len(final_gradients)} non-zero, max norm: {max_gradient_norm:.6f}")
        
        # Cache the result
        self._save_to_cache(final_gradients, cache_key)
