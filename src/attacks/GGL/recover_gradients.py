    ...
    with torch.no_grad():
        for epoch in range(epochs):
            for imgs, _ in tqdm(data_loader, desc=f"Computing covariance (epoch {epoch+1}/{epochs})"):
                ...
                with torch.no_grad():
                    for layer in conv_fea_dict:
                        f = features[conv_fea_dict[layer]].cpu()
                        patch = F.unfold(f, ks, dilation=1, padding=pad, stride=1)
                        patch = patch.permute(0, 2, 1).reshape(-1, patch.shape[1])
                        patch_covar = patch.T @ patch
                        
                        if layer not in covar:
                            covar[layer] = patch_covar
                        else:
                            covar[layer] += patch_covar
...
def calculate_svd_difference(covar_original, covar_unlearned, device, k=None, method='direct'):
    """
    ...
    Args:
        ...
        k: top-k components
        method: 'direct' for direct subtraction, 'svd_diff' for SVD-based difference
    """
    diff_svd = {}
    ...
    for layer in covar_original:
        ...
        elif method == 'svd_diff':
            # SVD-based difference (your original approach)
...
# REPLACE THIS FUNCTION in recover_gradients.py
def project_onto_subspace(covar_target, svd_basis, k=None, projection_type='full'):
    """
    Project target covariances onto subspaces defined by SVDs.
    
    Args:
        covar_target: dict of covariance matrices
        svd_basis: dict of SVD results with 'U' and 'S' keys
        k: number of top components to use
        projection_type: 'full', 'right', 'left', or 'orthogonal'
    """
    projected_covar = {}
    ...
    for layer in covar_target:
        ...
        if projection_type == 'full':
            # Full projection (both row and column spaces) - best for covariance matrices
            C_proj = P @ C_target @ P
        elif projection_type == 'right':
            # Right projection only (your original)
            C_proj = C_target @ P
        elif projection_type == 'left':
            # Left projection only
            C_proj = P @ C_target
        elif projection_type == 'orthogonal':
            # Orthogonal projection (like the paper's gradient projection)
            C_proj = C_target - P @ C_target @ P
        else:
...
def compare_projections(proj1, proj2, k=None, plot=False, id='comparison'):
    """
