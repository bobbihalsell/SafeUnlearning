...
class BOReconstructor():
    ...
    def reconstruct(self, input_gradient, use_pbar=True):
        ...
        labels = torch.tensor([812], device=input_gradient[0].device)  # Force label to 812 (space shuttle)

        # Ensure labels is a 1D tensor (batch of size 1)
        if labels.dim() == 0:
            labels = labels.unsqueeze(0)  # Convert scalar tensor to 1D tensor

        print('Inferred label new: {}'.format(labels))
    ...
    def ng_loss(z, # latent variable to be optimized
        ...
        if labels.dim() == 0:
            labels = labels.unsqueeze(0)  # Convert scalar to (1,)
        ...
        if labels.dim() == 0:
            labels = labels.unsqueeze(0)  # Convert scalar to (1,)
        labels = labels.to(dtype=torch.long)  # Ensure labels is LongTensor

        #target_loss, _, _ = loss_fn(fl_model(x), labels)
        target_loss = loss_fn(fl_model(x), labels)  # Ensure it's a single value
