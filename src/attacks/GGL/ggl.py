...
def calculate_grads(num_classes=1000, dim=1280, lr=0.01, model_old_path="mobilenet_80ep_imagenet.pth", model_new_path="../../../unlearning/gascent/mobilenet_unlearned_grad_ascent_1.pth"):
    # Initialize two MobileNetV2 models
    model_old = models.mobilenet_v2(pretrained=False)
    model_new = models.mobilenet_v2(pretrained=False)
...
def return_image(budget, model, generator, gt_grads, savepath):
    ...
    ng_rec = NGReconstructor(fl_model=model, generator=generator, loss_fn=loss_fn,
                                        num_classes=1000, search_dim=(128,), strategy='CMA', budget=budget, use_tanh=True, defense_setting=None)
...
if __name__=="__main__":
    ...
    gt_grads, model_new, model_old = calculate_grads(num_classes, dim=1280, lr=lr, model_old_path=model_old_path, model_new_path=model_new_path)
