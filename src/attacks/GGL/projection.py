...
def test_layer_names(dataloader1, dataloader2, original_model, unlearned_model, device, loss_fn=nn.CrossEntropyLoss(), variance_threshold=0.9):
    subspace = find_subspace(original_model, unlearned_model, dataloader1, device, variance_threshold=variance_threshold, cache_dir='test_layer_names_cache')
    param_difference = compute_parameter_difference(original_model, unlearned_model, cache_dir='test_layer_names_cache')
    gradients = get_model_gradients(unlearned_model, dataloader2, device, loss_fn, cache_dir='test_layer_names_cache')
    debug_layer_names(subspace, param_difference, gradients, dict_name="data", show_details=True)
