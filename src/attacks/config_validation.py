...
from cfg_validator import ConfigError, InputValidator
...
class ReconstructorValidator(InputValidator):
    ...
    def _validate_wandb_params(self):
        ...
        self.extra_config = (
            self._require(self.wandb_file, "extra_config")
            if self.wandb_file.get("extra_config")
            else {}
        )
        self.run_id = (
            self._require(self.wandb_file, "run_id")
            if self.wandb_file.get("run_id")
            else None
        )
    ...
    def _load_labels(self, labels_cfg):
        ...
        if isinstance(labels_cfg, str) and os.path.isfile(labels_cfg):
            try:
                ...
                raise ConfigError(
                    f"Invalid label format in {labels_cfg}. Expected integers."
                )
            ...
            if all(isinstance(label, int) for label in labels_cfg):
                ...
                raise ConfigError(
                    f"Invalid label format in {labels_cfg}. Expected a list of integers."
                )
        else:
            return None
