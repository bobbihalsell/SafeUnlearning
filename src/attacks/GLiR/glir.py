...
class GLiR:
    def __init__(
        ...
        if num_params is not None:
            ...
            self.indices = np.linspace(0, num_model_params - 1, num_params, dtype=int)
        else:
    ...
    def establish_baseline(self, points):
        ...
        """
        print(
            f"Establishing baseline using {len(points)} points with method '{self.method}'"
        )
        ...
        self.sigma = (feature_vars_norm.t() @ feature_vars_norm) / len(
            feature_vars_norm
        )
        ...
        try:
            ...
            sigma_reg = (
                self.sigma
                + torch.eye(self.sigma.shape[0], device=self.sigma.device) * 1e-4
            )
            ...
            sigma_reg = (
                self.sigma
                + torch.eye(self.sigma.shape[0], device=self.sigma.device) * 1e-3
            )
    ...
    def plot_glir_distribution(
        ...
        if labels is not None:
            ...
            if len(labels) != len(p_values):
                raise ValueError(
                    "Number of labels must match number of p_values/test_statistics"
                )
        ...
        if labels is not None:
            ...
            ax1.scatter(
                test_statistics,
                chi2.pdf(test_statistics, self.df),
                color="g",
                alpha=0.7,
                label="Test Statistics",
            )
            ax1.plot(
                test_statistics, np.zeros_like(test_statistics), "|", color="g", ms=20
            )
        ...
        # Threshold line
        ax2.axvline(
            x=alpha, color="g", linestyle="--", label=f"alpha={alpha} threshold"
        )
