...
class GLiR:
    ...
    def compute_test_statistic(self, x, y):
        """
        Compute GLiR test statistic for a data point
        """
        grad_before = self.compute_gradient(self.model_before, x, y)
        grad_after = self.compute_gradient(self.model_after, x, y)
        ...
        grad_diff = grad_diff[self.valid_indices]
        
        centered_diff = grad_diff - self.mean_vector.squeeze(0)

        # Compute the Mahalanobis distance using the inverse covariance matrix
        mahalanobis_distance = torch.sum(
            centered_diff * (self.sigma_inv @ centered_diff)
            )

        # Compute the Likelihood Ratio Test Statistic (LRT)
        lrt_statistic = 2 * mahalanobis_distance
        
        return lrt_statistic

    def compute_p_value(self, lrt_statistic, df=1):
        """
        Compute the p-value using the chi-squared distribution with 1 degree 
            of freedom.
        This assumes the test statistic follows a chi-squared distribution.
        args:
            df: int = 1 Degrees of freedom for the chi-squared distribution 
                (typically 1 for this kind of test)
        """
