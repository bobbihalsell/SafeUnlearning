...
# Replace your project_gradients function with this debug version:
...
def project_gradients(target_gradients, svd_basis, k=None):
    ...
    print(f"target_gradients: {len(target_gradients)} keys")
    print(f"svd_basis: {len(svd_basis)} keys")
    ...
    # Show first few keys
    print(f"Target gradient keys: {list(target_gradients.keys())[:5]}")
    print(f"SVD basis keys: {list(svd_basis.keys())[:5]}")
    ...
    # Process overlapping parameters only
    for param_name in overlap:
        grad_tensor = target_gradients[param_name]
        svd_result = svd_basis[param_name]
        
        # Check for None values
        if grad_tensor is None:
            print(f"SKIP {param_name}: grad_tensor is None")
            skipped_count += 1
            continue
            
        if svd_result is None:
            print(f"SKIP {param_name}: svd_result is None")
            skipped_count += 1
            continue
        
        # Check SVD structure
        if 'U' not in svd_result:
            print(f"SKIP {param_name}: no 'U' in svd_result")
            skipped_count += 1
            continue
        ...
        if U is None:
            print(f"SKIP {param_name}: U is None")
            skipped_count += 1
            continue
        
        if processed_count < 5:  # Debug first 5
            print(f"PROCESSING {param_name}:")
            print(f"  grad_tensor shape: {grad_tensor.shape}")
            print(f"  U shape: {U.shape}")
        
        if k is not None:
            U = U[:, :k]
            if processed_count < 5:
                print(f"  U truncated to: {U.shape}")
        
        # DTYPE FIX: Ensure both tensors have same dtype
        if grad_tensor.dtype != U.dtype:
            if processed_count < 5:
                print(f"  Converting U from {U.dtype} to {grad_tensor.dtype}")
            U = U.to(grad_tensor.dtype)
        
        # Create projection matrix P = U @ U.T
        try:
            P = U @ U.T
            if processed_count < 5:
                print(f"  P shape: {P.shape}")
        except Exception as e:
            print(f"ERROR creating P for {param_name}: {e}")
            error_count += 1
            continue
        ...
        if len(grad_tensor.shape) == 2:  # [sz, features] or [out_features, in_features]
            try:
                proj_grad = grad_tensor @ P
                result = grad_tensor - proj_grad
                projected_gradients[param_name] = result
                processed_count += 1
            ...
            if processed_count < 5:
                print(f"  SKIP: unexpected shape {grad_tensor.shape}")
            projected_gradients[param_name] = grad_tensor  # Skip if unexpected shape
            skipped_count += 1
    
    print(f"project_gradients SUMMARY:")
    print(f"  Processed: {processed_count}")
    print(f"  Skipped: {skipped_count}")
    print(f"  Errors: {error_count}")
    print(f"  Total output: {len(projected_gradients)}")
